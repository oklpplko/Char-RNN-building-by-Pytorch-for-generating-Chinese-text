...
# rnn.load_state_dict(torch.load('./new_trained/rnn_novel101.pkl'))
#rnn = torch.load('./new_trained/rnn_novel101.pkl').cuda()
...
for epoch in range(EPOCH):
    ...
    for batch in range(input_indexes.shape[0]):
        ...
        if batch % 200 == 0:
            print("Epoch: {}".format(epoch),
                  "| Batch: {}".format(batch),
                  "| loss: {:.4f}".format(loss.data.cpu().numpy().min()),
    ...
    if epoch % 2 == 0:
        ...
        print('\n'*2, "Epoch: {}".format(epoch), "| loss1: {:.4f}".format(loss1.data.cpu().numpy().min()))
